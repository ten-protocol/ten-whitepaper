...
Example: _R_15[Alice, 100, 102, 20]_ means the generation is 15, the aggregator is _Alice_, the generation of the L1 bock used as proof is 100, the generation of the L1 bock that included the rollup is 102 , and the nonce=20
...
In a L1 protocol, the gossiping of the winning block is the final step in deciding the winner of the round. The first node to get the block across to the majority of their peers wins the round.

In the L2 POBI protocol, the act of winning the gossip stage does not mean that the rollup is in the final ledger until it is also successfully published in a L1 block.

A malicious or malfunctioning aggregator that has not won the round could also publish the rollup they produced thus creating a competition. The management contract has to accept both since it can't know whether one of them is an actual invalid rollup generated by a hacked TEE. From the point of view of the L1, a mistake by an aggregator host and a TEE hack are indistinguishable.

These are the possible competition scenarios:

#### Competing rollups from the same proof mined in the same L1 block.

This scenario can happen if gossiping didn't function properly, or if the winner didn't gossip on purpose, or if an aggregator malfunctioned or decided to ignore the information received. Malicious behaviour is indistinguishable from honest errors. The second rule, designed to handle this case, provides no incentive for any of the participants to act maliciously, while still paying some reward assuming honesty.
...
a. The winner is in the earliest block. This case is simple. The winner is both on the canonical chain and receives the full reward.

b. The winner is in the later block. One other reason why this could happen is front-running, which is discouraged.

The rollup from the earliest block is on the canonical chain. Based on how many blocks are in between the rollups, the rewards might vary. According to rule #4, if the blocks are consecutive, pay a reduced reward only to the rollup with the lower nonce. According to rule #5, if the blocks are not consecutive, pay a full reward only to the rollup with the higher nonce.

[comment]: <> ([TODO - diagrams])

#### Competing rollups from different L1 proofs mined in the same L1 block.

Round N had a winner rollup _WRn_, but the rollup was not included in the next L1 block. Round N+1 had a winner _WRn+1_, which is included in the next L1 block, together with _WRn_

This case is handled by rule #3, which states that _WRn+1_ will be both on the canonical chain and will get the full reward, while _WRn_ will not be paid any reward.
...
TODO - does it matter the block difference? a. _WRn_ is included in an earlier block _WRn_ is on the canonical chain
...
If the rollup is valid, then all aggregators will use it as the parent of the current rollup, basically it will be on the canonical chain. The reward
