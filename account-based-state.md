...
To implement these requirements, Obscuro supports a runtime largely compatible with the EVM and is based on [geth](https://github.com/ethereum/go-ethereum), the canonical implementation of the Ethereum protocol.

On a high level, the OVM will be almost identical to the EVM, but there are significant differences behind the scenes between the data structures used by Ethereum and the ones used by Obscuro. The rollup structure also differs significantly from the Ethereum block structure as it contains encrypted transactions, plaintext withdrawal instructions, plaintext events, references to L1 blocks, and many more. The OVM will eventually introduce new abstractions and primitives to reflect the data privacy requirements and implement data isolation where it is needed. One last significant difference from the EVM is the implementation of the Obscuro cryptography requirements.
...
For example, for an individual (non-contract) account, the main rule could be formalized as:
*"Only the account owner, or someone empowered by the owner, is allowed to query the balance of an account."*
...
User wallets created transactions encrypted with the shared _Obscuro public key_. Only valid TEEs in possession of the private key can decrypt, execute, and see the resulting state. Still, end-users who submitted a transaction must be able to receive the result and query the balance.
